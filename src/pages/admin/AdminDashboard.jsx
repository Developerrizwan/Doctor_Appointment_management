import React, { useEffect, useState } from 'react'
import api from '../../api/axios'
import { parseApiError } from '../../api/errors'

const StatCard = ({ label, value }) => (
  <div className='bg-white border rounded-lg p-5 min-w-[8rem] flex-1'>
    <p className='text-2xl font-semibold text-gray-800'>{value}</p>
    <p className='text-gray-500 text-sm mt-1'>{label}</p>
  </div>
)

// Simple horizontal bar (no chart library).
const Bar = ({ label, value, max }) => (
  <div className='flex items-center gap-3 text-sm'>
    <span className='w-40 truncate text-gray-600'>{label}</span>
    <div className='flex-1 bg-gray-100 rounded h-4'>
      <div
        className='bg-blue-500 h-4 rounded'
        style={{ width: `${max ? (value / max) * 100 : 0}%` }}
      />
    </div>
    <span className='w-8 text-right text-gray-700'>{value}</span>
  </div>
)

const AdminDashboard = () => {
  const [data, setData] = useState(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')

  useEffect(() => {
    let ignore = false
    const load = async () => {
      try {
        const { data } = await api.get('/dashboard/admin/')
        if (!ignore) setData(data)
      } catch (err) {
        if (!ignore) setError(parseApiError(err, 'Could not load dashboard.'))
      } finally {
        if (!ignore) setLoading(false)
      }
    }
    load()
    return () => { ignore = true }
  }, [])

  if (loading) return <p className='text-gray-500'>Loading dashboard…</p>
  if (error) return <p className='text-red-500'>{error}</p>

  const specMax = Math.max(1, ...data.charts.by_speciality.map((s) => s.count))

  return (
    <div className='flex flex-col gap-6'>
      <h2 className='text-xl font-semibold text-gray-800'>Overview</h2>

      <div className='flex flex-wrap gap-4'>
        <StatCard label='Doctors' value={data.totals.doctors} />
        <StatCard label='Patients' value={data.totals.patients} />
        <StatCard label='Appointments' value={data.totals.appointments} />
        <StatCard label='Revenue' value={`$${data.revenue}`} />
      </div>

      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        {/* By speciality */}
        <div className='bg-white border rounded-lg p-5'>
          <p className='font-medium text-gray-800 mb-4'>Appointments by speciality</p>
          <div className='flex flex-col gap-2'>
            {data.charts.by_speciality.length === 0 && (
              <p className='text-sm text-gray-500'>No data yet.</p>
            )}
            {data.charts.by_speciality.map((s) => (
              <Bar key={s.speciality} label={s.speciality} value={s.count} max={specMax} />
            ))}
          </div>
        </div>

        {/* Over time */}
        <div className='bg-white border rounded-lg p-5'>
          <p className='font-medium text-gray-800 mb-4'>Appointments over time</p>
          <div className='flex flex-col gap-1 text-sm max-h-64 overflow-auto'>
            {data.charts.appointments_over_time.length === 0 && (
              <p className='text-gray-500'>No data yet.</p>
            )}
            {data.charts.appointments_over_time.map((d) => (
              <div key={d.date} className='flex justify-between border-b py-1'>
                <span className='text-gray-600'>{d.date}</span>
                <span className='text-gray-800 font-medium'>{d.count}</span>
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* Latest appointments */}
      <div className='bg-white border rounded-lg p-5'>
        <p className='font-medium text-gray-800 mb-4'>Latest appointments</p>
        <div className='flex flex-col gap-2'>
          {data.latest_appointments.length === 0 && (
            <p className='text-sm text-gray-500'>No appointments yet.</p>
          )}
          {data.latest_appointments.map((a) => (
            <div key={a.id} className='flex items-center justify-between text-sm border-b py-2'>
              <div className='flex items-center gap-3'>
                <img className='w-8 h-8 rounded-full object-cover bg-blue-50' src={a.doctor.image} alt='' />
                <div>
                  <p className='text-gray-800'>{a.patient?.name} → {a.doctor.name}</p>
                  <p className='text-gray-500 text-xs'>{a.slot_date} | {a.slot_time}</p>
                </div>
              </div>
              <StatusPill a={a} />
            </div>
          ))}
        </div>
      </div>
    </div>
  )
}

const StatusPill = ({ a }) => {
  let text = 'Booked'
  let cls = 'bg-blue-50 text-blue-600'
  if (a.cancelled) { text = 'Cancelled'; cls = 'bg-red-50 text-red-600' }
  else if (a.is_completed) { text = 'Completed'; cls = 'bg-green-50 text-green-600' }
  else if (a.payment) { text = 'Paid'; cls = 'bg-stone-100 text-stone-600' }
  return <span className={`text-xs px-2 py-1 rounded-full ${cls}`}>{text}</span>
}

export default AdminDashboard
