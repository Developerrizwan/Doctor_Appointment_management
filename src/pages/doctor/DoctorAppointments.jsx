...
const DoctorAppointments = () => {
  ...
  const complete = async (id) => {
    ...
    try {
      ...
      const msg = parseApiError(err, 'Could not complete.')
      setError(msg)
      toast.error(msg)
    } finally {
      setActingId(null)
        ...
        <table className='w-full text-sm'>
          ...
          <tbody>
            {appointments.map((a) => {
              ...
              return (
                <tr key={a.id} className='border-t'>
                  ...
                  <td className='p-3 text-gray-500'>{a.payment ? 'Paid' : 'Unpaid'}</td>
                  <td className='p-3'>
            ...
            {appointments.length === 0 && (
              <tr><td colSpan={6} className='p-4 text-center text-gray-500'>No appointments.</td></tr>
            )}
          </tbody>
        </table>
      </div>
